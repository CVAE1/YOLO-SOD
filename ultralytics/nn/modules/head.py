...
class Detect_Aux(nn.Module):
    """YOLOv8 Detect head with an auxiliary head for detection models."""
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.reg_max = 16  # DFL channels (ch[0] // 16 to scale 4/8/12/16/20 for n/s/m/l/x)
        self.no = nc + self.reg_max * 4  # number of outputs per anchor
        self.stride = torch.zeros(self.nl * 2)  # strides computed during build
        c2, c3 = max((16, ch[0] // 4, self.reg_max * 4)), max(ch[0], min(self.nc, 100))  # channels
        self.cv2 = nn.ModuleList(
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
            x[i+self.nl] = torch.cat((self.cv4[i](x[i+self.nl]), self.cv5[i](x[i+self.nl])), 1)
        if self.training:  # Training path
            return x
        ...
        if self.dynamic or self.shape != shape:
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(x[:self.nl], self.stride[:self.nl], 0.5))
            self.shape = shape
